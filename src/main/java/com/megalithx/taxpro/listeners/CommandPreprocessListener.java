...
public class CommandPreprocessListener implements Listener {
    ...
    public void onCommandPreprocess(PlayerCommandPreprocessEvent event) {
        ...
        String message = event.getMessage().substring(1); // remove slash
        String[] args = message.split(" ");
        String label = args[0].toLowerCase();

        // Handle alias plugins (like sending /essentials:heal)
        if (label.contains(":")) {
        ...
        if (fee != null && fee.price > 0) {
            ...
            // Bypass the fee completely if their balance is under the exemption threshold
